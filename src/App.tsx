...
import { Cpu, Settings as SettingsIcon, MessageSquare, Terminal, Globe, Menu, LayoutDashboard, Monitor, Wrench, Headphones, GitBranch } from 'lucide-react';
import { config } from './config';

type ViewId = 'chat' | 'agent-os' | 'mission-control' | 'remote-devices' | 'settings' | 'memory' | 'skills' | 'vault' | 'canvas' | 'marketplace' | 'websearch' | 'cron' | 'security-agent' | 'knowledge-graph' | 'talk' | 'self-improve' | 'api-keys' | 'agents-hub' | 'devices-hub' | 'tools-hub' | 'admin-hub' | 'browser' | 'workspaces' | 'ceo' | 'vision' | 'notifications' | 'git';

interface SubView { id: ViewId; label: string; icon: React.ElementType; desc: string; }

const CATEGORIES: { id: ViewId; label: string; icon: React.ElementType; direct?: ViewId; subs: SubView[] }[] = [
...
function AppContent() {
  ...
  return (
    <VoiceProvider>
    <div className="app-root" style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100vh', overflow: 'hidden', fontFamily: 'var(--font-sans)', background: '#000000', color: '#ffffff' }}>
      {/* Mobile Top Bar */}
      <div className="mobile-top-bar" style={{ display: 'none', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', paddingTop: 'calc(12px + env(safe-area-inset-top, 0px))', borderBottom: '1px solid rgba(255,255,255,0.05)', backgroundColor: '#050505', zIndex: 45 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Cpu size={20} color="#fff" />
          <span style={{ fontSize: '18px', fontWeight: 800 }}>{config.APP_NAME}</span>
        </div>
        <button onClick={() => setMobileMenuOpen(true)} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', padding: 4 }}>
          <Menu size={24} />
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {/* Desktop Sidebar */}
        <nav className={`desktop-sidebar ${mobileMenuOpen ? 'mobile-open' : ''}`} style={{ width: '200px', height: '100%', padding: '24px 12px', display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.05)', backgroundColor: '#050505', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '0 8px 24px' }}>
            ...
            <span style={{ fontSize: '16px', fontWeight: 800 }}>{config.APP_NAME}</span>
          </div>
              ...
              return (
                <button key={cat.id} onClick={() => navigateTo(cat.id)}
                  ...
                  style={{
                    ...
                    border: 'none', borderRadius: '6px', cursor: 'pointer', textAlign: 'left', fontSize: '13px', fontWeight: 500,
      ...
      <nav className="mobile-bottom-nav" style={{
        ...
        borderTop: '1px solid rgba(255,255,255,0.05)', backgroundColor: '#050505', zIndex: 45,
      }}>
        {BOTTOM_TABS.map(tab => {
          ...
          return (
            <button key={tab.id} onClick={() => handleBottomTab(tab.id)}
              ...
              style={{
                ...
                background: 'none', border: 'none', color: isTabActive ? '#fff' : '#555',
              ...
              <tab.icon size={20} style={{ opacity: isTabActive ? 1 : 0.5 }} />
              <span>{tab.label}</span>
            </button>
          );
        })}
